...
function renderMiniBlocks(blocks) {
  return `<div class="hint-blocks">${blocks.map(b => {
    if (b.type === 'graph') {
      // Extrait le ratio viewBox pour adapter la hauteur
      const m = (b.svgContent || '').match(/viewBox=['"]([^'"]+)['"]/);
      let ratioStyle = '';
      if (m) {
        const parts = m[1].trim().split(/\s+|,/);
        if (parts.length >= 4) {
          const r = (parseFloat(parts[2]) / parseFloat(parts[3])).toFixed(4);
          if (r) ratioStyle = `style="--graph-ratio:${r}"`;
        }
      }
      return `
        <div class="hint-block hint-block--graph">
          <div class="hint-block__title">${b.title}</div>
          <div class="hint-block__graph" ${ratioStyle}>
            <div class="graph-svg-wrap">${b.svgContent || ''}</div>
          </div>
